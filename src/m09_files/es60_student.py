...
# Funzioni - Validazione
# def valida_voto(voto_str) -> list[bool, str | float]
#     """Valida il voto. Restituisce (è_valido, valore_o_errore)."""

# Funzioni - I/O Formattazione
# def formatta_lista(studenti) -> str
#     """Formatta la lista degli studenti come stringa. Restituisce l'output formattato."""

# def mostra_dettaglio(studenti, indice) -> list[bool, str, dict | None]
#     """Mostra il dettaglio di uno studente. Restituisce (successo, messaggio, dati)."""


def valida_voto(voto_str) -> list[bool]:
    voto = float(voto_str)
    if 1 <= voto <= 10:
        return True 
    else:
...
def formatta_lista(studenti) -> None:
    for studente in studenti:
        print(f"\nL'alunno/a {studente['nome']} ha preso {studente['voto']} ")
...
# Funzioni - Operazioni CRUD
# def aggiungi_studente(studenti) -> list[bool, str, dict | None]
#     """Aggiunge uno studente. Restituisce (successo, messaggio, nuovo_studente)."""

# def cancella_studente(studenti) -> list[bool, str]
#     """Elimina uno studente per indice. Restituisce (successo, messaggio)."""

# def aggiorna_studente(studenti) -> list[bool, str]
#     """Aggiorna il voto di uno studente. Restituisce (successo, messaggio)."""

def aggiungi_studente(studenti) -> None:
    nome_studente =str(input("Inserisci il nome del nuovo studente: "))
    voto_studente = float(input("Inserisci il voto del nuovo studente: "))
    studenti.append({"nome":nome_studente, "voto":voto_studente})
    print("Studente aggiunto correttamente!")
    print(studenti)
    

def cancella_studente(studenti) -> None:
    while True:
        studente_eliminato=str(input("Inserisci il nome dello studente che vuoi eliminare con la prima lettera maiuscola: "))
        for studente in studenti:
            if studente_eliminato != studente["nome"]:
                print("Non esiste uno studente con questo nome ")
            else:
...
def main():
    ...
    # voto_str = input("Inserisci un voto:")
    # if valida_voto(voto_str) == True:
    #     print("Il voto è valido")
    # else:
